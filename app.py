...
def get_user_api_key(user_id):
    """
    Get the decrypted API key for a user from database.
...
def get_current_api_key():
    ...
    3. For authenticated users, returns None if no API key is found
    4. Falls back to environment variable only for unauthenticated requests (single-user mode)
    ...
    if user_id:
        # For authenticated users, return their API key or None
        api_key = get_user_api_key(user_id)
        if api_key:
            return api_key
        else:
            # User is authenticated but has no API key configured
            return None

    # Fallback to environment variable only for non-authenticated requests (single-user mode)
...
def save_api_key(api_key):
    ...
    if user_id:
        # Multi-tenant mode: save to database
        try:
            ...
            if existing:
                ...
                execute(
                    ...
                    (encrypted_key, 'Default Key', user_id)
                )
            else:
                # Insert new
                execute(
        ...
        # Single-user mode: save to .env file
...
@bp.route('/api/user/has-api-key', methods=['GET'])
def check_user_has_api_key():
    """Check if the authenticated user has an API key configured."""
    try:
        ...
        logger.error(f"Error checking API key: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': 'Failed to check API key'}), 500
...
def api_key_management():
    ...
    if token:
        try:
            ...
            logger.warning(f"Invalid token in /api/key: {e}")
            # Continue without authentication for backwards compatibility with single-user mode
    ...
    if request.method == 'GET':
        ...
        if user_id:
            # Multi-tenant mode: delete from database
            try:
                from database import execute

                execute(
                    "DELETE FROM user_api_keys WHERE user_id = %s",
                    (user_id,)
                )

                logger.info(f"API key deleted from database for user {user_id}")
            ...
            # Single-user mode: delete from .env
            env_file = find_dotenv()
